...
def getResponse(message) -> str:
    ...
    if (userMessage == "nyan"):
        ...
        embed.color = discord.Color.from_rgb(255, 192, 203)
        embed.set_image(
            url="https://media1.tenor.com/m/9fV87rRDzDgAAAAC/ansubin0925hellomynameissoobin.gif")
        embed.title = "Cat! :3"
        return (embed)
    ...
    if (userMessage == ":3 help"):
        ...
        embed.add_field(
            name="Setup", value="**:3 setup**: Set Channel To Allow Cat Requests Within!",
            inline=False)
        embed.add_field(
            name="Interval", value="**:3 timer {amount}**: Set Amount Of Times Per Day Cats Are Automatically Sent!", inline=False)
        embed.add_field(
            name="", value="**:3 timer remove**: Disable The Automatic Sending Of Cats!",
            inline=False)
        embed.add_field(
            name="", value="**:3 timer current**: See The Rate At Which The Automatic Sending Of Cats Occurs!",
            inline=False)
        embed.add_field(
            name="Usage", value="**:3 cat or :3 meow**: Request Another Cat!", inline=False)
        embed.add_field(name="", value="**:3 dog or :3 woof**: Request A Dog Friend!",
                        inline=False)
        embed.add_field(name="Response Messages",
                        value="nyan, meow, :3, woof, and arf!", inline=False)
        return (embed)
    ...
    if (userMessage == "negative interval"):
        ...
        embed.color = discord.Color.from_rgb(255, 192, 203)
        embed.add_field(
            name="", value="You're Smart! But Uh I Can't Do Math... Please Enter A Non-Negative Number! :3")
        return (embed)
